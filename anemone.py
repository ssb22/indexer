#!/usr/bin/env python3
"""
Anemone 1.78 (http://ssb22.user.srcf.net/anemone)
...
the number of threads you specify (which might also be
useful if you don't want to use all available CPU cores).""")
...
class Run():
  ...
  def write_all(self,recordingTexts) -> None:
    ...
    if R.mp3_recode and any(R.audioData) or any(
        ...
        if R.max_threads: executor = ThreadPoolExecutor(
                max_workers=R.max_threads)
...
shared_executor = ThreadPoolExecutor(max_workers=cpu_count())
