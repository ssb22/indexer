...
if(document.getElementsByTagName) {
  ...
  d.appendChild(s);
  s.innerHTML = "\u01d0"; // i3
  b.appendChild(d); var iWidth = s.offsetWidth; b.removeChild(d);
  s.innerHTML = "\u01d2"; // o3
  b.appendChild(d); var oWidth = s.offsetWidth; b.removeChild(d);
  var supports_pinyin = oWidth > iWidth; // (if proportional)
  if (supports_pinyin) {
    var links=document.getElementsByTagName("A");
    for(var i=0; i<links.length; i++) if(links[i].innerHTML && links[i].getAttribute("href").indexOf("gradint.cgi?jsl=zh&")>-1) {
      ...
      if (n!=o) { links[i].innerHTML=n; if(links[i].style)links[i].style.fontFamily="Gandhari, DejaVu Sans, Lucida Sans Unicode, Times New Roman, serif"; }
    }
  }
}
} nums2marks();
